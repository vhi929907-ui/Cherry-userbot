...
API_URL = "https://shuruuuuuuuuuuuu-num-info.vercel.app/apis/num_info_v1"

async def num_cmd(client, message):
    if len(message.command) < 2:
        await message.edit("Usage: .num <phone_number>")
        return

    number = message.command[1]

    await message.edit("🔍 Fetching number information...")
    ...
    try:
        ...
        if not data.get("status"):
            await message.edit("❌ No information found.")
            return

        result = data["result"]["0"]

        name = result.get("name", "N/A")
        fname = result.get("fname", "N/A")
        num = result.get("num", "N/A")
        alt = result.get("alt", "N/A")
        circle = result.get("circle", "N/A")
        email = result.get("email") or "N/A"
        aadhaar = result.get("aadhar", "N/A")
        address = result.get("address", "N/A").replace("!", "\n")

        if aadhaar != "N/A" and len(aadhaar) >= 8:
            aadhaar = aadhaar[:4] + "****" + aadhaar[-4:]
...
new_user.add_handler(
    MessageHandler(
        num_cmd,
        filters.command("num", prefixes=".") & filters.me
    )
)
